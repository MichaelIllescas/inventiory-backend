...
public interface SaleRepository extends JpaRepository<Sale, Long> {
     ...
     */
    @Query("SELECT s FROM Sale s WHERE s.saleDate = :date AND s.user.id = :userId AND s.status = com.imperial_net.inventioryApp.sales.model.SaleStatus.CONFIRMED")
     ...
     */
    @Query("SELECT s FROM Sale s WHERE s.saleDate BETWEEN :startDate AND :endDate AND s.user.id = :userId AND s.status = com.imperial_net.inventioryApp.sales.model.SaleStatus.CONFIRMED")
    ...
    @Query("SELECT c.name, c.lastname, c.documentNumber, SUM(s.totalSale), COUNT(s.id) " +
            ...
            "JOIN s.customer c " +
            "WHERE s.saleDate BETWEEN :startDate AND :endDate AND s.user.id = :userId " +
            "AND s.status = com.imperial_net.inventioryApp.sales.model.SaleStatus.CONFIRMED " +
